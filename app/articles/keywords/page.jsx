...
export default function NewMom() {
    ...
    const breadcrumbItems = [
        { label: 'Keywords' },
        { label: 'Article Search Keywords', href: '/articles/keywords' },
    ];
    ...
    useEffect(() => {
        if (!fetchedRef.current) {
            ...
            fetchArticleKeywords(1, rowsPerPage, { sortField, sortOrder });
        }
    }, []);
    const fetchArticleKeywords = async (pageNum = 1, limit = 10, options = {}) => {
        setIsLoading(true);
        const payload = {
            params: {
        ...
        try {
            const data = await apiRequest(apiRoutes.articleSearchlist, 'POST', payload, router);
            if (data?.response) {
                const articleList = data?.data?.docs;
    ...
    const handleSortChange = (field, direction) => {
        ...
        fetchArticleKeywords(page + 1, rowsPerPage, { sortField: field, sortOrder: direction });
    };
    const handleTableChange = ({ newPage, newRowsPerPage }) => {
        ...
        fetchArticleKeywords(finalPage + 1, finalRowsPerPage, { ...form, sortField, sortOrder });
    };
    ...
    //     }));
    //     fetchArticleKeywords(page + 1, rowsPerPage, { sortField, sortOrder, ...filterValues },);
    // }
    const handleFilterSubmit = (filterValues) => {
        setFilterOpen(false);
        ...
        fetchArticleKeywords(newPage + 1, rowsPerPage, { sortField, sortOrder, ...filterValues });
    };
