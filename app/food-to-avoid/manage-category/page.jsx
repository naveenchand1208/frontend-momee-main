...
export default function Manage_Category() {
    ...
    const breadcrumbItems = [
        ...
        { label: 'Category', href: '/food-to-avoid/manage-category' }
    ];
    const breadcrumbAction = [
        ...
        //             fetchCategories(1, rowsPerPage, filters);
        //             setStatusLabel('Active/Inactive');
        //         } else {
        //             setForm((prev) => ({ ...prev, status: val }));
        //             fetchCategories(1, rowsPerPage, { ...form, status: val });
        //             setStatusLabel(val);
        //         }
        //     },
        // },
    ...
    const fetchCategories = async (pageNum = 1, limit = 5, options = {}) => {
        setIsLoading(true);
        setDisableLoadingForEdit(false);
        const payload = {
            params: {
    ...
    const handleDeleteConfirm = async () => {
        ...
        const data = await apiRequest(apiRoutes.deleteFoodAvoidCategory, 'POST', payload, router);
        if (data.response) {
    ...
    const handleToggleStatus = async (updatedRow) => {
        ...
        try {
            const data = await apiRequest(apiRoutes.updateFoodAvoidCategory, 'POST', formData, router);
            if (data.response) {
                fetchCategories(currentPage + 1, currentRowsPerPage, { ...filterForm, sortField, sortOrder });
            }
        } catch (error) {
    ...
    const handleIndexSubmit = async (filterValues) => {
        ...
        if (Object.keys(filterValues) && filterValues.index !== '') {
            ...
            try {
                const data = await apiRequest(apiRoutes.updateIndexFoodAvoidCategory, 'POST', payload, router);
                if (data.response) {
                    fetchCategories(page + 1, rowsPerPage, { sortField, sortOrder });
                    setIndexOpen(false)
                }
    ...
    const handleFilterSubmit = (filterValues) => {
        ...
        // always start from page 1 after applying filter
        fetchCategories(1, rowsPerPage, { sortField, sortOrder, ...updatedFilter });
        setFilterOpen(false);
    };
    // const handleFilterSubmit = (filterValues) => {
    //     setFilterOpen(false)
    ...
    const manageSubscription = async (formData) => {
        const action = !isEdit
            ? apiRoutes.addFoodAvoidCategory
            : apiRoutes.updateFoodAvoidCategory;
        try {
