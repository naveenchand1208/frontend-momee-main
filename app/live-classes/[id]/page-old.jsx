...
export default function AddLiveClasses() {
    ...
    const viewLiveClasses = async (id) => {
        try {
            ...
            if (data?.response) {
                ...
                setForm({
                    name: live?.name || '',
                    nameTa: live?.translations?.ta?.name || '',
                    file: live?.file || '',
                    performedBy: live?.performedBy || '',
                    performedByTa:
                    ...
                    MeetingLink: live?.MeetingLink || '',
                    MeetingLinkTa:
                        live?.translations?.ta?.MeetingLink || '',
                    description: live?.description || '',
                    descriptionTa:
    ...
    if (
        ...
        showError(
            'Please fill in all required English and Tamil fields.'
        );
    ...
    if (updatedForm.deviceType === 'ios') {
        ...
        const checkAmount = IOS_AMOUNT_OPTIONS.find(
            option => option.value === updatedForm.amount
        );
    ...
    if (isEdit) {
        ...
        const isFileChanged =
            updatedForm.file !== viewLiveClass.file;
    ...
    const formData = objectToFormData(finalForm);

    // IMPORTANT - explicitly add all required fields

    formData.set('name', updatedForm.name);
    formData.set('nameTa', updatedForm.nameTa);

    formData.set('performedBy', updatedForm.performedBy);
    formData.set('performedByTa', updatedForm.performedByTa);

    formData.set('description', updatedForm.description);
    formData.set('descriptionTa', updatedForm.descriptionTa);

    // ONLY ENGLISH MEETING LINK
    formData.set('MeetingLink', updatedForm.MeetingLink);

    formData.set('fromDate', updatedForm.fromDate);
    formData.set('toDate', updatedForm.toDate);

    formData.set('startTime', updatedForm.startTime);
    formData.set('endTime', updatedForm.endTime);

    formData.set('momType', updatedForm.momType);
    formData.set('status', updatedForm.status);

    formData.set('amount', updatedForm.amount);
    formData.set('deviceType', updatedForm.deviceType);
                    ...
                    <div className="custom-form row" style={{ width: '100%', height: 'max-content' }}>
                        ...
                        <div className="mt-2 col-12 col-sm-6 col-md-4">
                            <Input
                                ...
                                name="performedByTa"
                                value={form.performedByTa}
                                onChange={(e) =>
                                    setForm({
                                        ...form,
                                        performedByTa: e.target.value
                                    })
                                }
                                required={true}
                                formSubmitted={formSubmitted}
