...
export default function Diet_Plans_Add() {
    const { id } = useParams();
    const isEdit = id !== 'add';
    ...
    const breadcrumbItems = [
        ...
        { label: isEdit ? 'Edit' : 'Add', href: isEdit ? `/diet-plans/${id}` : '/diet-plans/add' }
    ];
    ...
    const handleDeviceTypeView = (value) => {
        setForm((prevForm) => ({
            ...prevForm,
            deviceType: value,
        }));
    ...
    const viewDietPlan = async (id) => {
        try {
            ...
            if (data?.response) {
                ...
                handleDeviceTypeView(plan?.deviceType)
                setViewform(plan);
                setIsLoading(false);
            }
        } catch (error) {
    ...
    const manageDietPlan = async (payload) => {
        const action = isEdit ? apiRoutes.updateDietSubscription : apiRoutes.addDietSubscription;

        try {
            const data = await apiRequest(action, 'POST', payload, router);
            if (data?.response) {
                showSuccess(isEdit ? 'Diet Plan updated successfully!' : 'Diet Plan added successfully!');
                // handleClear();
                router.push('/diet-plans');
            } else {
                setButtonLoading(false);
                ...
                <form ref={formRef} onSubmit={handleSubmit}>
                    <div className="row">
                        ...
                        <div className="col-md-12 mb-2">
                            {/* <label className="form-label" style={{ fontSize: '0.875rem' }}>Device Type</label> */}
                            <RadioGroup
                                label="Device Type"
                                name="deviceType"
