...
import { SUBSCRIPTION } from '@/common/constants/enum';
import { formatDate, getMonths } from '@/common/utils/util';
...
export default function NewMom() {
  ...
  const userListHeaders = [
    ...
    { id: 'completedMonths', label: 'Months', sortable: true },
        ...
        const userList = data?.data?.docs?.map((item) => ({
          ...
          subscriptionDetails: item?.subscriptions?.length > 0
            ? [item.subscriptions[0]?.subscribedPlan?.planName, item.subscriptions[0]?.validaityEndDate]
              ...
              .join(' ') || 'Not Subscribed'
            : '-',
          completedMonths: item.completedMonths
            ? `Month ${Number(item.completedMonths)}`
            : '-',
...
//   const handleFilterSubmit = (filterValues) => {
  ...
  //   // 🔍 Step 1: Find the plan object from planOptions using the label (name)
  //   const selectedPlan = planOptions.find(plan => plan.label === filterValues.plan);

  //   // showError('Please upload a valid audio file') Step 2: Prepare a new object to send to the API
  //   // Use the plan ID (value) instead of the label (name) for backend filtering
  //   const apiPayload = {
  //     ...filterValues,
  //     plan: selectedPlan?.value || '', // send ID to API
  //   };

  //   // 💾 Step 3: Update form state for the filter UI
  //   // Keep the label (plan name) so that Autocomplete still shows it properly
  //   setForm(prev => ({
  //     ...prev,
  //     searchKey: filterValues.searchKey || '',
  //     status: filterValues.status || '',
  //     subscribed: filterValues.subscribed || '',
  //     plan: selectedPlan?.label || '', // store plan name, NOT ID
  //     month: filterValues.month || '',
  //   }));

  //   // 📡 Step 4: Call fetchUsers with converted payload (ID, not name)
  //   fetchUsers(page + 1, rowsPerPage, {
  //     sortField,
  //     sortOrder,
  //     ...apiPayload, // includes correct plan ID
  //   });
  // };