...
export default function Diet_Food_Eat() {
    const { id } = useParams();
    ...
    const fetchDietFoodEatList = async (pg = 1, limit = 10, sort = {}, catList = categories) => {
        ...
        try {
            ...
            const data = await apiRequest(apiRoutes.dateWiseDietFood, 'POST', payload, router);
    ...
    const viewDietFood = async (id) => {
        try {
            ...
            const data = await apiRequest(apiRoutes.viewDietFood, 'POST', payload, router);
            if (data?.response) {
                return data.data;
            }
        } catch (error) {
